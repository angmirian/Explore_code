...
WSLS = FALSE #Set to TRUE if want to run the WSLS analyses, FALSE if not
trial_data = ("/Users/angela/Documents/Work/Explore/Medusa_analysis/BSocial_Replication/trial_df.csv") #("/Users/angela/Documents/Work/Explore/Medusa_analysis/trial_df_03282023.rds")
subject_demographics = ("dfx3_filt.RData") #("/Users/angela/Documents/Work/Explore/Medusa_analysis/explore_complete_demographics.rds")
lethalithy_data = ("bsocial_clock_suihx.csv")
...
setwd("/Users/angela/Documents/Work/Explore/Medusa_analysis/BSocial_Replication")
#Define how output files are named
if (subjs_to_run=="all") {
  if (select_networks==TRUE) {
    output_file_specifier = "_SelectNetworks"
  } else {output_file_specifier = ""}
} else if (subjs_to_run=="controls_only") {
  if (select_networks==TRUE) {
    output_file_specifier = "_CONTROLS_ONLY_SelectNetworks"
  } else {output_file_specifier = "_CONTROLS_ONLY"}
} else if (subjs_to_run=="patients_only") {
  if (select_networks==TRUE) {
    output_file_specifier = "_PATIENTS_ONLY_SelectNetworks"
  } else {output_file_specifier = "_PATIENTS_ONLY"}
} else if (subjs_to_run=="attempters_only") {
  if (select_networks==TRUE) {
    output_file_specifier = "_ATTEMPTERS_ONLY_SelectNetworks"
  } else {output_file_specifier = "_ATTEMPTERS_ONLY"}
}
...
if (align_to=="clock") {
    models<-read.table("/Users/angela/Documents/Work/Explore/Medusa_analysis/Decode_formulas/clock_decode_formulas_9Dec2025_Bsocial_Replication_pid5_impulsivity.txt")
} else if (align_to=="response") {
    models<-read.table("/Users/angela/Documents/Work/Explore/Medusa_analysis/Decode_formulas/feedback_decode_formulas_9Dec2025_Bsocial_Replication_pid5_impulsivity.txt") #from Explore paper analyses?
}
decode_formula = list()
...
if (WSLS) {
  ########## Extract Random Subject-Level Slopes ##########
  ##Create ID column to be added at end
  #excludes = c(207224, 210374, 210701, 213708, 216806, 216845, 219392, 220024, 221698, 440149, 440311, 440336) # for Explore paper
  #trial_df <- trial_df %>% filter(!id %in% excludes)
  
  #trial_df_wide <- trial_df %>% distinct_at(vars(id), .keep_all=TRUE)
  #ID <- trial_df_wide %>% dplyr::select(id)
  #ID_sorted <- arrange(ID,id)
  
  ##reward_lag_rec and rt_lag_sc interactions as random slope (with reward_lag_rec coded as -0.5 and 0.5 to decorrelate main effect and interaction)
  #Sep 2024 - change from trial w/in condition to trial w/in run
  #m100 <- lmer(rt_csv ~ rt_lag_sc*reward_lag_rec + rt_vmax_lag_sc*condition_trial_neg_inv + rt_lag_sc*condition_trial_neg_inv +
  #               (rt_lag_sc*reward_lag_rec|id) + (1|id:run),trial_df)
  m100 <- lmer(rt_csv ~ rt_lag_sc*reward_lag_rec + rt_vmax_lag_sc*trial_neg_inv + rt_lag_sc*trial_neg_inv +
                 (rt_lag_sc*reward_lag_rec|id) + (1|id:run),trial_df)
  while (any(grepl("failed to converge", m100@optinfo$conv$lme4$messages) )) {
    print(m100@optinfo$conf$lme4$conv)
    ss <- getME(m100,c("theta","fixef"))
    m100 <- update(m100, start=ss)}
  car::Anova(m100,'3')
  #summary(m100)
  #vif(m100)
  m100_coef_ranef <- ranef(m100)
  m100_coef_ranef1 <- m100_coef_ranef[["id"]]
  ##Add ID column
  #m100_coef_ranef2 <- bind_cols(ID_sorted,m100_coef_ranef1) # error prone
  m100_coef_ranef2 <- m100_coef_ranef1 #Try w/o bind_cols b/c it is error prone (Sep 2024)
  m100_coef_ranef2$id <- rownames(m100_coef_ranef2)
  #cor.test(m100_coef_ranef2$`rt_lag_sc:reward_lag_rec`,m100_coef_ranef2$rt_lag_sc)
  
  ##Do same for omissions only
  trial_df_OM <- trial_df %>% filter(reward_lag_rec=="-0.5")
  m100o <- lmer(rt_csv ~ rt_vmax_lag_sc*trial_neg_inv + rt_lag_sc*trial_neg_inv +
                  (rt_lag_sc|id) + (1|id:run),trial_df_OM)
  while (any(grepl("failed to converge", m100o@optinfo$conv$lme4$messages) )) {
    print(m100o@optinfo$conf$lme4$conv)
    ss <- getME(m100o,c("theta","fixef"))
    m100o <- update(m100o, start=ss)}
  car::Anova(m100o,'3')
  #summary(m100o)
  #vif(m100o)
  
  m100o_coef_ranef <- ranef(m100o)
  m100o_coef_ranef1 <- m100o_coef_ranef[["id"]]
  ##Add ID column
  #m100o_coef_ranef2 <- bind_cols(ID_sorted,m100o_coef_ranef1) # error prone
  m100o_coef_ranef2 <- m100o_coef_ranef1 #Try w/o bind_cols b/c it is error prone (Sep 2024)
  m100o_coef_ranef2$id <- rownames(m100o_coef_ranef2)
  
  ## Investigate how different approaches differ (WSLS including all trials vs. LS for omission trials only)
  cor.test(m100_coef_ranef2$rt_lag_sc,m100o_coef_ranef2$rt_lag_sc)
  plot(-m100_coef_ranef2$`rt_lag_sc:reward_lag_rec`,m100o_coef_ranef2$rt_lag_sc)
  #combined_rs_df <- data.frame(m100_coef_ranef2$`rt_lag_sc:reward_lag_rec`,m100o_coef_ranef2$rt_lag_sc) 
  selected_m100_coef_ranef2 <- m100_coef_ranef2 %>% select(id,rt_lag_sc) %>% rename(WSLS_rs = rt_lag_sc)
  selected_m100o_coef_ranef2 <- m100o_coef_ranef2 %>% select(id,rt_lag_sc) %>% rename(LS_rs = rt_lag_sc)
  combined_rs_df <- left_join(selected_m100_coef_ranef2,selected_m100o_coef_ranef2,by="id")
  cor(combined_rs_df$WSLS_rs, combined_rs_df$LS_rs)
  # Join combined_rs_df with trial_df
  combined_rs_df$id <- as.double(combined_rs_df$id)
  trial_df <- trial_df %>% left_join(combined_rs_df, by="id")

  # WSLS and LS by group
  combined_rs_df_wGroups <- trial_df %>% select(id, registration_group, groupLeth, WSLS_rs, LS_rs) %>% unique()
  # Plot WSLS and LS by group
  p_WSLS <- ggplot(combined_rs_df_wGroups,
                   aes(x = groupLeth, y = WSLS_rs,
                       color = groupLeth)) +
    stat_summary(fun = mean, geom = "point", size = 4) +
    stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.1) +
    labs(
      x = "Registration Group",
      y = "WSLS_rs",
      title = "WSLS_rs by groupLeth"
    ) +
  ...
  p_LS <- ggplot(combined_rs_df_wGroups,
                   aes(x = groupLeth, y = LS_rs,
                       color = groupLeth)) +
    stat_summary(fun = mean, geom = "point", size = 4) +
    stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.1) +
    labs(
...
subcortical_cache_dir="/Users/angela/Documents/Work/Explore/Medusa_analysis/BSocial_Replication/subcortical"   #"/Users/angela/Documents/Work/Explore/Medusa_analysis/subcortical"
cortical_cache_dir="/Users/angela/Documents/Work/Explore/Medusa_analysis/BSocial_Replication/cortical"  #"/Users/angela/Documents/Work/Explore/Medusa_analysis/BSocial_Replication/cortical"
if (align_to=="clock") {
  print("aligning to clock")
  HCmedusa_data = c("/Users/angela/Documents/Work/Explore/Medusa_analysis/BSocial_Replication/clock_aligned_bsocial_hippocampus_resample.csv.gz")
  cortical_and_subcortical_data = "/Users/angela/Documents/Work/Explore/Medusa_analysis/BSocial_Replication/clock_aligned_bsocial_vmpfc_subcortical_resample.csv.gz"
  ...
  HCmedusa_data = c("/Users/angela/Documents/Work/Explore/Medusa_analysis/BSocial_Replication/feedback_aligned_bsocial_hippocampus_resample.csv.gz")
  cortical_and_subcortical_data = "/Users/angela/Documents/Work/Explore/Medusa_analysis/BSocial_Replication/feedback_aligned_bsocial_vmpfc_subcortical_resample.csv.gz"
...
if (region_to_run=="cortical") {
  cortical_df = fread(cortical_and_subcortical_data)
  ...
  Q <- merge(trial_df, cortical_df, by = c("id", "run", "trial")) %>% dplyr::arrange("id","run","trial","evt_time") #takes a long time
  Q$decon_mean[Q$evt_time > Q$iti_ideal] = NA
} else if (region_to_run=="subcortical") { 
  subcortical_df = fread(cortical_and_subcortical_data)
  ...
  HCdata=fread(file.path(HCmedusa_data)) # 
  ...
  avg_hipp <- HCdata %>% group_by(id,run,trial,evt_time) %>% 
    dplyr::summarize(hipp_avg_decon_mean=mean(decon_mean,na.rm=TRUE),
                     hipp_avg_decon_median=mean(decon_median,na.rm=TRUE),
                     hipp_avg_decon_sd=mean(decon_sd,na.rm=TRUE)) %>% ungroup()
  ...
  avg_hipp <- avg_hipp %>% dplyr::rename(decon_mean=hipp_avg_decon_mean,
                                           decon_median=hipp_avg_decon_median,
                                           decon_sd=hipp_avg_decon_sd)
  #Add atlas values for HC data before combining
  avg_hipp$atlas_value <- 500
  ...
  subcortical_df <- rbind(subcortical_df, avg_hipp)
  #rm(HCdata1)
  #rm(HCdata2)
  ...
  cobraHC_labels <- data.frame(roi_num=c(500),hemi=c('B'),network=c('Hippocampus_cobra'),
                               subregion=c('Hippocampus_cobra'),x=c(0),y=c(0),z=c(0),
                               MNI_Glasser_HCP_v1.0=c('Hippocampus_cobra'),
                               Brainnetome_1.0=c('Hippocampus_cobra'),
                               CA_ML_18_MNI=c('Hippocampus_cobra'),
                               atlas_value=c(500)) 
  ...
  Q <- merge(trial_df, subcortical_df, by = c("id", "run", "trial")) %>% dplyr::arrange("id","run","trial","evt_time")
  #Remove cerebellum data
  Q <- Q %>% filter(!subregion=="Cerebellum")
  Q$decon_mean[Q$evt_time > Q$iti_ideal] = NA
}
Q$groupLeth <- as.factor(Q$groupLeth)
#Filter based on subject group you are running
if (subjs_to_run=="patients_only") {
  Q <- Q %>% filter(!groupLeth == "HC")
  Q$groupLeth <- droplevels(Q$groupLeth)
} else if (subjs_to_run=="controls_only") {
  Q <- Q %>% filter(groupLeth == "HC")
  Q$groupLeth <- droplevels(Q$groupLeth)
}
...
Q <- Q %>% rename(sex=registration_birthsex,
                  education_yrs = registration_edu,
                  Group=registration_group)
Q$female <- relevel(as.factor(Q$sex),ref="1")
Q$age <- scale(Q$age)
Q$pid5_impulsivity <- scale(Q$pid5_impulsivity)
#Q$wtar <- scale(Q$wtar)
#Q$exit <- scale(Q$exit)
...
Q$group_leth <- factor(Q$group_leth, level = c("HC", "BPD_NON", "BPD_LL", "BPD_HL"))
Q$Group <- factor(Q$Group, level = c("HC", "NON", "ATT"))

Q$group_leth_HL <- factor(Q$group_leth, level = c("BPD_HL", "HC", "BPD_NON", "BPD_LL"))
Q$group_leth_LL <- factor(Q$group_leth, level = c("BPD_LL", "HC", "BPD_NON", "BPD_HL"))
Q$Group_a <- factor(Q$Group, level = c("ATT", "HC", "NON"))
