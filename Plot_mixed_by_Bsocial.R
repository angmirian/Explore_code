...
#Need to change name of data_subfolder each time you run this
#Updated 5/2/2025 to make lines thicker
...
subcortical_cache_dir = '/Users/angela/Documents/Work/Explore/Medusa_analysis/BSocial_Replication/subcortical/' 
cortical_cache_dir = '/Users/angela/Documents/Work/Explore/Medusa_analysis/BSocial_Replication/cortical/'

if (region_to_run=="cortical") {
  data_subfolder <- "Patients_only/pid5_impulsivity" 
} else if(region_to_run=="subcortical") {
  data_subfolder <- "Selected_subcortical_regions/Patients_only/pid5_impulsivity" 
}
...
for (i in 1:length(data_files$V1)) {
  ...
  } else if(toplot == "estimate") {ylabel <- 'Coefficient (a.u.)'}
  ...
  #color_options <- c('#53a6c2','#c912ab','#e17014','#32af64','#644bc8','blue','firebrick1','darkseagreen1','gold') #for option 1 with lighter background
  color_options <- c('#53a1c2','#c912ab','#e17014','#32af64','#644bc8','blue','firebrick1','darkseagreen1','gold') #for option 2 with lighter background
  
  
  if (region_to_run == "subcortical") {
    color_scheme <- c(color_options[1:length(unique(ddf$network))])
    
    #Changed May 2, 2025 to address Michael's comment
    ##original options
    ...
    ##option with darker background
    #background_color='gray70'
    #major_grid_color='gray90' 
    #minor_grid_color='gray70' 
    #hline_color='white' 
    #vline_color='white'
    
    ###Option 1 with lighter background
    #background_color='gray95'
    #major_grid_color='gray80' 
    #minor_grid_color='gray95' 
    #hline_color='gray40' 
    #vline_color='gray40'
    
    ###Option 2 with lighter background
    background_color='gray90'
    major_grid_color='gray80' 
    minor_grid_color='gray90' 
    hline_color='gray40' 
    vline_color='gray40'
  ...
  for (fe in terms) {
    ...
    gg<-ggplot(edf, aes(x=t, y=.data[[toplot]], color=network, group=network)) + #changed to flip y for groups compared to attempters 5/16 x=t, y=statistic, color=network)) + 
      ...
      geom_line(aes(group=network, alpha = `p, FDR-corrected`), size=3, position=pd, show.legend = FALSE) + #line size was 1.5
    ...
    if (align_to == "response") {gg <- gg + scale_x_continuous(breaks = seq(0, 2, by = 1))} 
    gg <- gg  + theme_bw(base_size=13) +
      #xlab(paste("Time with respect to\n",align_to,"(seconds)")) + #commented out 5/2/2025
      #ylab(ylabel) + # commented out 5/2/2025
      theme(legend.title = element_blank(),
            axis.title.x = element_blank(), # added 5/2/2025
            axis.title.y = element_blank(), # added 5/2/2025
            panel.grid.major = element_line(colour = major_grid_color), 
            panel.grid.minor = element_line(colour = minor_grid_color), 
            panel.background = element_rect(fill = background_color),
            #axis.title.y = element_text(size=25, margin=margin(r=6)), # added size 5/2/25
            #axis.title.x = element_text(size=25, margin=margin(t=6)), # added size 5/2/25
            axis.text.y = element_text(size=20), #added 5/2/25
            axis.text.x = element_text(size=20)) #added 5/2/25
    print(gg)
    dev.off()
  }
}
